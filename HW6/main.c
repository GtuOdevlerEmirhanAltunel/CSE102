 ...
 * 
 */
void printMap(int oyuncular[OYUNCU_SAYISI], int cezalar[][2])
{
    int playGround[ROWS][COLS];
            ...
            for (int x = 0; x < CEZA_SAYISI; x++) 
                if (i == cezalar[x][0] && j == cezalar[x][1] && i != 0)
                {
                    if (temp >= 49 && temp <= 57)
                    {
                        ...
                        return printMap(oyuncular, cezalar); // TODO Restart printMap funtions with changed values.
                    }
...
void start_game(int siralama[OYUNCU_SAYISI])
{
    ...
    while (devam)
    {
        if (devam != 2) // ? Super long printf to not use brackets.
            printf("Decide who starts first. \e[%dm%d\e[0m player mode.\n(You can change the number of the players in the game settings.)", OYUNCU_SAYISI + 30, OYUNCU_SAYISI);
